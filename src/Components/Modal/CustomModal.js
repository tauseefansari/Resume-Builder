...
const CustomModal = ({ showModal, modalContent }) => {
  ...
  return (
    <Modal
      ...
      BackdropProps={{ timeout: ANIMATION_TIMEOUT }}
    >
            ...
            {modalContent.custom ? (
              <>
                <Box sx={{ display: "flex", justifyContent: customDataCheck ? "space-between" : "flex-end", mt: 1 }}>
                  {customDataCheck && (
                    <Button variant="outlined" size="small" color="error" startIcon={<DynamicIcon iconName="Delete" />} onClick={handleRemoveSection}>
                      Remove Last
                    </Button>
                  )}
                  <Button variant="outlined" size="small" startIcon={<DynamicIcon iconName="Add" />} onClick={handleAddSection}>
                    Add More
                  </Button>
                </Box>
